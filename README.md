...
**Note**: If the script returns a value of type `string`/`boolean`/`number`, that return value will be placed to the `result` field. If the return value is an `object`, its contents will be directly passed to the agent.
