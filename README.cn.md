...
* 简化了提示词管理
* 保持通用性和兼容性：按照功能组织调用提示词工程脚本库，追求独立于特定大模型的通用性，让脚本能灵活应用于各种不同的大模型(TODO)
* 让用户轻松上手：应用程序开发者可以直接使用提示词工程，就像使用任何常规代码库一样，无需深入了解复杂的AI内部机制，降低了使用门槛
* 清晰划分并能自定义和使用提示词类型：目前提示词脚本类型分为
  * lib: `lib`类型, 基础的提示词脚本库，服务于底层逻辑
...
user: "讲个笑话吧！"
assistant: "[[JOKE]]"
-> translator(target_lang="葡萄牙语") # 传入智能体的实际输入参数是: {content: "[这里是由AI生成的笑话]", target_lang: "葡萄牙语"}
```
